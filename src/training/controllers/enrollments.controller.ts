...
export class EnrollmentsController {
  ...
  @ApiResponse({ status: 201, description: 'Trainee enrolled successfully', type: CourseEnrollment })
  @ApiResponse({ status: 400, description: 'Bad request' })
  @ApiResponse({ status: 404, description: 'Course or trainee not found' })
  async enrollTrainee(@Body() enrollTraineeDto: EnrollTraineeDto): Promise<CourseEnrollment> {
    return await this.enrollmentsService.enrollTrainee(enrollTraineeDto);
  }

  @Get()
  @ApiOperation({ summary: 'Get all enrollments' })
  @ApiResponse({ status: 200, description: 'Return all enrollments', type: [CourseEnrollment] })
  ...
  @ApiQuery({ name: 'traineeId', required: false, description: 'Filter by trainee ID' })
  async findAll(
  ...
  @ApiResponse({ status: 200, description: 'Return an enrollment', type: CourseEnrollment })
  ...
  @ApiResponse({ status: 200, description: 'Enrollment updated successfully', type: CourseEnrollment })
  @ApiResponse({ status: 404, description: 'Enrollment not found' })
  @ApiResponse({ status: 400, description: 'Bad request' })
  async update(
    @Param('id') id: string,
  ...
  @ApiResponse({ status: 404, description: 'Enrollment not found' })
  @ApiResponse({ status: 400, description: 'Cannot delete completed enrollment' })
  ...
  @ApiResponse({ status: 200, description: 'Enrollment confirmed successfully', type: CourseEnrollment })
  @ApiResponse({ status: 404, description: 'Enrollment not found' })
  @ApiResponse({ status: 400, description: 'Only pending enrollments can be confirmed' })
  ...
  @ApiResponse({ status: 200, description: 'Enrollment cancelled successfully', type: CourseEnrollment })
  @ApiResponse({ status: 404, description: 'Enrollment not found' })
  @ApiResponse({ status: 400, description: 'Cannot cancel completed enrollment' })
