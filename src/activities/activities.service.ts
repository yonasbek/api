...
export class ActivitiesService {
  ...
  async removeSupportingDocument(
    ...
    if (!activity.supporting_documents?.includes(fileName)) {
      throw new BadRequestException(
        `File ${fileName} not found in activity's supporting documents`,
      );
    }
  ...
  async update(
    ...
    if (updateActivityDto.start_date && updateActivityDto.end_date) {
      if (
        new Date(updateActivityDto.start_date) >
        new Date(updateActivityDto.end_date)
      ) {
  ...
  async getBudgetSummary(planId: string): Promise<{
    ...
    const total_allocated = activities.reduce(
      (sum, activity) => sum + Number(activity.budget_allocated),
      0,
    );
    const total_spent = activities.reduce(
      (sum, activity) => sum + Number(activity.budget_spent),
      0,
    );
  ...
  async getProgressSummary(planId: string): Promise<{
    ...
    const total_progress = activities.reduce(
      (sum, activity) => sum + activity.progress,
      0,
    );
    const average_progress =
      total_activities > 0 ? Math.round(total_progress / total_activities) : 0;
